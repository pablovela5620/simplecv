...
def cut_episode(
    ...
    ep_output: Path = output_dir / ep_name / "synced"
    ...
    if ego_in.exists():
        ...
        # Cut videos
        ego_videos: list[str] = ["left.mp4", "right.mp4", "rgb.mp4"]
        for video in tqdm(ego_videos, desc=f"  Cutting ego videos ({ep_name})", leave=False):
            video_in: Path = ego_in / video
            video_out: Path = ego_out / video
            if video_in.exists():
                cut_video_ffmpeg_nvenc(video_in, video_out, start_s, end_s)
        ...
        # Copy calibration files unchanged
        calibration_files: list[str] = ["calibration.json", "recording_stats.json"]
    ...
    if exo_in.exists():
        ...
        # Iterate over each exo camera directory
        exo_cameras: list[Path] = [d for d in exo_in.iterdir() if d.is_dir()]
        for cam_dir in tqdm(exo_cameras, desc=f"  Cutting exo cameras ({ep_name})", leave=False):
            cam_name: str = cam_dir.name
            cam_out: Path = exo_out / cam_name
            cam_out.mkdir(exist_ok=True)

            # Find and cut the video file
            video_files: list[Path] = list(cam_dir.glob("*.mp4"))
            for video_in in video_files:
                # Output video uses simpler naming: CameraName.mp4
    ...
    if quest_in.exists():
        ...
        # Cut videos
        quest_videos: list[str] = ["left.mp4", "right.mp4"]
        for video in tqdm(quest_videos, desc=f"  Cutting quest videos ({ep_name})", leave=False):
