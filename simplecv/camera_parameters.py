...
class Extrinsics:
    ...
    def compute_transformation_matrices(self) -> None:
        ...
        if self.world_R_cam is not None and self.world_t_cam is not None:
            self.world_T_cam: Float[np.ndarray, "4 4"] = (
                self.compose_transformation_matrix(self.world_R_cam, self.world_t_cam)
            )
            self.cam_T_world: Float[np.ndarray, "4 4"] = np.linalg.inv(self.world_T_cam)
        ...
        # If camera-to-world is provided, compute the transformation matrix and its inverse
            ...
            self.cam_T_world: Float[np.ndarray, "4 4"] = (
    ...
    def decompose_transformation_matrix(
        ...
        t: Float[np.ndarray, "3 "] = T[:3, 3]  # noqa: UP037
        return R, t
...
class PinholeParameters:
    ...
    def compute_projection_matrix(self) -> None:
        ...
        self.projection_matrix: Float[np.ndarray, "3 4"] = (
