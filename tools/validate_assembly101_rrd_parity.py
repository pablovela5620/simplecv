...
#
# We fail on *missing* GT data (any missing chunk lineage that the GT has).
# Important nuance: the Assembly101 GT catalog files contain TWO stores —
# a `recording` store (~29 chunks: video assets + ego Transform3D) and a
# `blueprint` store (~98 chunks). The per-frame keypoint streams written
# by ``log_exoego_batch`` (``coco133_xyz``, ``coco133_uv``,
# ``simplecv.KeypointConfidence*``) land in the BLUEPRINT store rather
# than the recording store because ``rr.send_blueprint`` switches the
# active stream right before ``log_exoego_batch`` runs. The catalog
# server's ``schema.component_columns()`` only surfaces the recording
# store, which is why an earlier version of this validator silently
# missed those keypoint chunks entirely. The current implementation
# walks both stores via the low-level ``rerun_bindings`` reader.

# Keypoint-bearing entity-path substrings; presence of any indicates the
# `log_exoego_batch` keypoint stream was written.
KEYPOINT_PATH_MARKERS: tuple[str, ...] = (
    "coco133_xyz",
    "coco133_uv",
    "KeypointConfidence",
    "/world/gt/",
...
class RRDSummary:
    ...
    file_bytes: int
    # Across BOTH the recording and blueprint stores: total dynamic-row
    # count attributed to entities matching ``KEYPOINT_PATH_MARKERS``.
    # Treats them uniformly because the GT catalog lands them in the
    # blueprint store while a freshly-generated file may land them in the
    # recording store; either way they're part of the contract.
    keypoint_rows: int
    ...
    def from_rrd(cls, path: Path) -> "RRDSummary":
        ...
        try:
            ...
            sess.close()
        keypoint_rows: int = _count_keypoint_rows(path)
        return cls(
...
def _count_keypoint_rows(path: Path) -> int:
    """Sum the dynamic rows across BOTH stores for keypoint-bearing entities.

    Walks ``rerun_bindings.RrdReaderInternal.store_entries()`` and inspects
    each store's per-chunk ``summary()`` lines because:

    * the catalog server's ``schema.component_columns()`` only surfaces
      the recording store, but
    * ``log_exoego_batch`` keypoint streams in the Assembly101 GT files
      land in the *blueprint* store (the GT was generated when
      ``rr.send_blueprint`` had already switched the active stream).
    """
    reader = rb.RrdReaderInternal(str(path))
    total: int = 0
    for entry in reader.store_entries():
        store = reader.store(entry)
        summary: str = store.summary()
        for line in summary.split("\n"):
            if not any(marker in line for marker in KEYPOINT_PATH_MARKERS):
                continue
            # Each chunk-summary line looks like
            # ``/path rows=NNNNN static=False timelines=... cols=[...]``.
            idx = line.find("rows=")
            if idx < 0:
                continue
            tail = line[idx + len("rows=") :]
            end = tail.find(" ")
            n_str = tail if end < 0 else tail[:end]
            try:
                total += int(n_str)
            except ValueError:
                pass
...
def diff_summaries(cand: RRDSummary, gt: RRDSummary, candidate_path: Path, gt_path: Path) -> ParityResult:
    ...
    # Keypoint-row parity: if the GT carries any keypoint chunks (which it
    # does, in the blueprint store), the candidate must carry the same total
    # row count to within a few-frame tolerance. This is the check that the
    # earlier validator silently missed and the reason exp-12/17/19/20
    # turned out to be invalid.
    if gt.keypoint_rows > 0:
        delta = abs(cand.keypoint_rows - gt.keypoint_rows)
        if cand.keypoint_rows == 0:
            res.checks["keypoint_rows"] = (
                f"FAIL:GT has {gt.keypoint_rows} keypoint rows but candidate has 0"
            )
        elif delta > 24:  # allow a handful of frames difference
            res.checks["keypoint_rows"] = (
                f"FAIL:cand={cand.keypoint_rows} gt={gt.keypoint_rows} Δ={delta}"
            )
        else:
            res.checks["keypoint_rows"] = "PASS"
